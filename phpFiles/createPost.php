...
$sql = "INSERT INTO NEW_FEEDS (user_id, content, date_post, like_count, comment_count) VALUES (?, ?, ?, ?, ?)";

try {
    $stmt = $pdo->prepare($sql);
    $stmt->bindParam(1, $user_id);
    $stmt->bindParam(2, $content, PDO::PARAM_STR);  // Specify the parameter type
    $stmt->bindParam(3, $date_post);
    $stmt->bindParam(4, $like_count);
    $stmt->bindParam(5, $comment_count);
    $stmt->execute();
